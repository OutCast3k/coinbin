			...
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					if($(data).find("result").text()=="1"){
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html("Sent!<br /><br />txid: "+$(data).find("txid").text());
					} else {
		...
		$.ajax ({
			...
			error: function(data) {
				...
				if($(data).find("result").text()==1){
					...
					$("#rawTransactionStatus").html('Sent!<br /><br />txid: '+$(data).find("txid").text());
				} else {
